    ...
    overlay.innerHTML = `
      <div class="settings-panel">
        ...
        <div class="settings-footer">
          ...
          <button class="btn btn-secondary" id="export-config-btn">📤 Export</button>
          <button class="btn btn-secondary" id="import-config-btn">📥 Import</button>
...
Important: For non-English/non-Chinese comments, you MUST use the three-part format with labels.`;
  ...
  async function initPanel(forceShow = false) {
    ...
    collapseBtn.addEventListener('click', async (e) => {
      ...
      if (isCollapsing) {
        // 折叠：保存展开状态位置
        let savedX = Math.max(0, Math.min(currentX, winWidth - rect.width));
        let savedY = Math.max(0, Math.min(currentY, winHeight - rect.height));
      ...
      requestAnimationFrame(() => {
        ...
        let correctedX = Math.max(0, Math.min(newRect.left, winWidth - newRect.width));
        let correctedY = Math.max(0, Math.min(newRect.top, winHeight - newRect.height));
    ...
    importConfigFile.addEventListener('change', async (e) => {
      ...
      try {
        showToast('Importing config...');
        await importConfig(file);
        Logger.info('Config imported');
        showToast('Config imported successfully, reloading...');
